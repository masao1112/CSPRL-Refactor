...
class StationPlacement(gym.Env):
    ...
    def __init__(self, my_graph_file, my_node_file, my_plan_file, location="DongDa", obs_type="mlp", action_type="discrete"):
        ...
        self.row_length = 10  # 10 features per node
        ...
        if self.action_type == "discrete":
            self.action_space = spaces.Discrete(2 * len(self.node_list))
        elif self.action_type == "multidiscrete":
            self.action_space = spaces.MultiDiscrete([len(self.node_list), 2])
        else:
        ...
        if self.obs_type == "mlp":
            shape = self.row_length * len(self.node_list) + 3  # budget_scaled, progress_scaled, score_delta
    ...
    def step(self, my_action):
        ...
        """
        # Clean up any empty stations (defensive programming)
        ...
        if self.action_type == "discrete":
            if my_action < N:
                # Operation: Add capacity at node
                op = 0
                chosen_idx = my_action
            else:
                # Operation: Relocate capacity to node
                op = 1
                chosen_idx = my_action - N
        else:
            chosen_idx, op = my_action

        chosen_node = self.node_list[chosen_idx]
        station_list = [s[0][0] for s in self.plan_instance.plan]

        if op == 0:
            # Add capacity
            if chosen_node[0] not in station_list:
                # Build a new station using initial solution configuration
                ...
                # Add a charger column to existing station
                station_idx = None
                for station in self.plan_instance.plan:
                    if station[0][0] == chosen_node[0]:
                        station_idx = self.plan_instance.plan.index(station)
                        break
                
                if station_idx is not None and self.station_config_check(self.plan_instance.plan[station_idx]):
                    config_index = 3  # Add default 20 kW charger
                    self.budget_adjustment_small(chosen_node, config_index)
                    if not self.game_over:
                        self.plan_instance.plan[station_idx][1][config_index] += 1
        else:
            # Relocate capacity: steal from lowest-benefit station, add to chosen_node
            steal_plan = [s for s in self.plan_instance.plan if s[0][0] not in self.plan_instance.existing_plan]
            stolen_station = H.anti_choose_node_bybenefit(self.node_list, steal_plan)

            if stolen_station is not None:
                old_budget = self.budget
                self.budget, config_index = self.plan_instance.steal_column(stolen_station, self.budget)

                if chosen_node[0] not in station_list:
                    # Build new station with relocated charger
                    new_config = np.zeros(len(H.CHARGING_POWER))
                    new_config[config_index] = 1
                    station_instance = Station()
                    station_instance.add_position(chosen_node)
                    station_instance.add_chargers(new_config)
                    station_instance.establish_dictionnary(self.node_list)
                    ...
                    # Add relocated charger to existing station
                    station_idx = None
                    for station in self.plan_instance.plan:
                        if station[0][0] == chosen_node[0]:
                            station_idx = self.plan_instance.plan.index(station)
                ...
                # Robust fallback: if nothing can be stolen yet, treat as Add capacity (op == 0)
                if chosen_node[0] not in station_list:
                    default_config = H.initial_solution(self.config_dict, self.node_list, chosen_node)
                    station_instance = Station()
                    station_instance.add_position(chosen_node)
        ...
        # Recompute capabilities and details
        for station in self.plan_instance.plan:
        ...
        if self.game_over:
            ...
            reward += 2.0 * terminal_improvement
